...
class GroupManagement:
    ...
    async def mute_user(self, platform, group_id: int, user_id: int, duration: int = 0) -> bool:
    ...
    async def unmute_user(self, platform, group_id: int, user_id: int) -> bool:
        """解除禁言"""
        try:
            client = platform.get_client()
            await client.api.call_action('set_group_ban',
    ...
    async def global_mute(self, platform, group_id: int, duration: int = 43200) -> bool:
        """全体禁言"""
        try:
            client = platform.get_client()
    ...
    async def kick_user(self, platform, group_id: int, user_id: int) -> bool:
        """踢出用户"""
        try:
            client = platform.get_client()
